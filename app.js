...
function AquariumsPage(){return shell(`${head("Aquariums","Feeding and maintenance at a glance")}
  ...
  <div class="a18-aquarium-grid">${(data.aquariums||[]).map(tank=>{const m=lina17Maintenance(tank), water=lina17MaintenanceDue(m.waterChange,7), feed=lina17LatestFeed(tank);return `<button class="a18-tank-card" data-route="tank" data-route-id="${esc(tank.id)}">
    ...
    <span class="a18-tank-copy"><span class="a18-title-row"><strong>${esc(tank.name)}</strong><b>›</b></span>
      <span class="a18-stat-grid"><span><small>Temp</small><em class="a18-temp">${esc(tank.temperature||"—")}${tank.temperature&&!String(tank.temperature).includes("°")?"°C":""}</em></span><span><small>Feed</small><em class="${lina18CareTone(feed)}">${esc(feed)}</em></span><span><small>Water</small><em class="${lina18CareTone(water)}">${esc(water)}</em></span></span>
      <span class="a18-maint-grid"><span><small>Sponge</small><em>${esc(m.spongeChange?lina17RelativeCareDate(m.spongeChange,{pastLabel:"ago"}):"Not logged")}</em></span><span><small>Filter</small><em>${esc(m.filterChange?lina17RelativeCareDate(m.filterChange,{pastLabel:"ago"}):"Not logged")}</em></span></span>
    </span>
...
function AquariumTankPage(){const tank=(data.aquariums||[]).find(x=>x.id===routeId)||(data.aquariums||[])[0];if(!tank)return AquariumsPage();const m=lina17Maintenance(tank);const feeds=lina17SortedFeeds(tank);const now=new Date(),defaultTime=`${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}`;const jobs=[["waterChange","Water Change","💧",7],["clean","Tank Clean","🫧",14],["spongeChange","Sponge Filter Clean","🧽",21],["filterChange","Filter Media Clean","⚙️",42]];return shell(`${head(tank.name,"Aquarium care",true)}
  ...
  <section class="a18-temperature-card">
    ...
    <small class="a18-updated">${tank.temperatureUpdated?`Last updated: ${new Date(tank.temperatureUpdated).toLocaleString("en-GB",{dateStyle:"medium",timeStyle:"short"})}`:"Add the current tank temperature"}</small>
  </section>
  <section class="a18-action-list">
    <details class="a18-action-card"><summary><span class="a18-icon warm">🍽️</span><span><strong>Feed</strong><small>${feeds.length?`Last: ${esc(lina17LatestFeed(tank))}`:"No feeds logged"}</small></span><em class="a18-soon">${feeds.length?esc(lina17LatestFeed(tank)):"Add feed"}</em><b>›</b></summary><div class="a18-expand-content"><div class="feed-list">${feeds.length?feeds.map(({feed,index,date,time})=>`<div class="compact-feed-row"><span><strong>${esc(feed.food||feed.name||"Fed")}</strong><small>${date?formatDate(date):"Date not recorded"}${time?` · ${esc(time)}`:""}</small></span><button class="mini danger" data-delete-feed="${index}">×</button></div>`).join(""):"<p class='muted-copy'>No feeding entries yet.</p>"}</div><div class="feed-add-grid"><label><span>Date</span><input class="field" id="feedDate" type="date" value="${today()}" max="${today()}"></label><label><span>Time</span><input class="field" id="feedTime" type="time" value="${defaultTime}"></label><label class="feed-note-field"><span>Food / note</span><input class="field" id="feedName" placeholder="Food or feeding note"></label><button class="secondary" id="addFeed">Log feed</button></div></div></details>
    ${jobs.map(([key,label,icon,cycle])=>{const due=lina17MaintenanceDue(m[key],cycle);return `<details class="a18-action-card"><summary><span class="a18-icon">${icon}</span><span><strong>${label}</strong><small>${m[key]?`Last: ${formatDate(m[key])}`:"Not logged yet"}</small></span><em class="${lina18CareTone(due)}">${esc(due)}</em><b>›</b></summary><div class="a18-expand-content"><div class="a18-log-row"><input class="field" type="date" data-maintenance-date="${key}" value="${m[key]||today()}"><button class="secondary" data-save-maintenance="${key}">Log ${label.toLowerCase()}</button></div><div class="a18-history-list">${lina17DateList(m.history[key]).length?lina17DateList(m.history[key]).map(date=>`<div class="history-date-row"><span>${formatDate(date)}</span><button class="mini danger" data-delete-maintenance="${key}" data-date="${date}">×</button></div>`).join(""):"<p class='muted-copy'>No history yet.</p>"}</div></div></details>`}).join("")}
  </section>`,'pets')}
