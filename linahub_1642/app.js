...
function render(){
  ...
  requestAnimationFrame(()=>setupUniversalTileCustomisation());
...
function ensureUniversalTileData(){
  data.tileCustomisations=data.tileCustomisations||{};
  return data.tileCustomisations;
}

function universalTileKey(tile){
...
function applyUniversalTileCustomisation(tile){
  const key=universalTileKey(tile);
  tile.dataset.universalTileKey=key;
  tile.classList.add("customisable-tile");
  const custom=ensureUniversalTileData()[key];
  if(!custom) return;
  const {icon,title}=universalTileParts(tile);
  if(custom.name&&title) title.textContent=custom.name;
  if(custom.icon&&icon&&!icon.classList.contains("module-image")) icon.textContent=custom.icon;
  if(custom.accent){
    tile.style.setProperty("--universal-tile-accent",custom.accent);
    tile.classList.add("has-custom-accent");
  }
}
...
function universalTileEditor(tile){
  ...
  const custom=ensureUniversalTileData()[key]||{};
  const {icon,title}=universalTileParts(tile);
  const originalName=(title?.textContent||"Tile").trim();
  const originalIcon=icon&&!icon.classList.contains("module-image")?(icon.textContent||"✨").trim():"✨";
  const currentName=custom.name||originalName;
  const currentIcon=custom.icon||originalIcon;
  const currentAccent=custom.accent||"#9d61ff";
  ...
  mount.innerHTML=`<div class="tile-editor-backdrop universal-tile-editor-backdrop" data-close-universal-tile-editor>
    <section class="tile-editor universal-tile-editor" role="dialog" aria-modal="true" aria-labelledby="universalTileEditorTitle">
      ...
      <div class="emoji-quick-picks" aria-label="Quick icon choices">${["✨","❤️","🌙","🔮","🌿","🌸","💊","😴","⚖️","📏","🏡","🐠","🎮","✈️","💷","📖"].map(value=>`<button type="button" data-quick-icon="${value}">${value}</button>`).join("")}</div>
      <label class="tile-colour-label">Accent colour<input type="color" id="universalTileAccent" value="${esc(currentAccent)}"></label>
      <div class="tile-editor-actions"><button type="button" class="secondary" id="universalTileReset">Reset tile</button><button type="button" class="primary" id="universalTileSave">Save changes</button></div>
    </section>
  </div>`;
  document.body.appendChild(mount);
  const close=()=>mount.remove();
  mount.addEventListener("click",event=>{if(event.target.matches("[data-close-universal-tile-editor]"))close()});
  const nameInput=mount.querySelector("#universalTileName");
  const iconInput=mount.querySelector("#universalTileIcon");
  const accentInput=mount.querySelector("#universalTileAccent");
  const preview=mount.querySelector(".universal-tile-preview");
  const updatePreview=()=>{
    preview.querySelector("span").textContent=iconInput.value.trim()||"✨";
    preview.querySelector("strong").textContent=nameInput.value.trim()||originalName;
    preview.style.setProperty("--universal-tile-accent",accentInput.value);
  };
  nameInput.addEventListener("input",updatePreview);iconInput.addEventListener("input",updatePreview);accentInput.addEventListener("input",updatePreview);
  mount.querySelectorAll("[data-quick-icon]").forEach(button=>button.onclick=()=>{iconInput.value=button.dataset.quickIcon;updatePreview()});
  mount.querySelector("#universalTileReset").onclick=()=>{delete ensureUniversalTileData()[key];saveData();close();render();toast("Tile reset")};
  mount.querySelector("#universalTileSave").onclick=()=>{
    ensureUniversalTileData()[key]={name:nameInput.value.trim().slice(0,40)||originalName,icon:iconInput.value.trim().slice(0,12)||originalIcon,accent:accentInput.value};
    saveData();close();render();toast("Tile updated ✨");
  };
}
...
if("serviceWorker" in navigator){
  window.addEventListener("load",async()=>{
    try{
      const registration=await navigator.serviceWorker.register("./sw.js?v=1638",{updateViaCache:"none"});
