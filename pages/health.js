...
function HealthPage(){
 ...
 const dashboard=`<section class="card health-dashboard-hero"><span class="section-kicker">❤️ Your health today</span><h2>${new Date().toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long"})}</h2><div class="health-dashboard-stats"><div><small>Sleep</small><strong>${morning.sleep!=null?HEALTH_FEELINGS.sleep[morning.sleep][0]:"—"}</strong><small>${latestSleep?minutesLabel(latestSleep.totalMinutes):"No time logged"}</small></div><div><small>Energy</small><strong>${morning.energy!=null?HEALTH_FEELINGS.energy[morning.energy][0]:"—"}</strong></div><div><small>Mood</small><strong>${morning.mood!=null?HEALTH_FEELINGS.mood[morning.mood][0]:"—"}</strong></div><div><small>Pain</small><strong>${morning.pain!=null?HEALTH_FEELINGS.pain[morning.pain][0]:"—"}</strong></div></div></section><section class="health-module-grid"><button data-health-open="sleep"><span>${morning.sleep!=null?HEALTH_FEELINGS.sleep[morning.sleep][0]:moduleArt("sleep","😴","health-tile-art")}</span><strong>Sleep</strong><small>${latestSleep?minutesLabel(latestSleep.totalMinutes):"Choose how you slept"}</small></button><button data-route="medication">${moduleArt("medication","💊","health-tile-art")}<strong>Medication</strong><small>Today, history & stock</small></button><button data-route="period">${moduleArt("period","🌸","health-tile-art")}<strong>Period</strong><small>Cycle & predictions</small></button><button data-health-open="weight">${moduleArt("weight","⚖️","health-tile-art")}<strong>Weight</strong><small>${latest?`${esc(healthWeightValue(latest))} kg`:"Add weight"}</small></button><button data-health-open="measurements">${moduleArt("measurements","📏","health-tile-art")}<strong>Measurements</strong><small>${measures.length} entries</small></button></section>`;
