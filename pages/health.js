...
function HealthPage(){
 ...
 <section class="card"><h2>Add measurements</h2>
 ...
 <section class="card"><h2>Measurement history</h2><div class="list-card">${measures.length?measures.map((m,i)=>`<div class="item-row"><div><h3>${esc(formatDate(m.date))}</h3><p>Waist ${m.waist||"—"} cm · Tummy ${m.tummy||"—"} cm</p></div><button class="mini danger" data-measure-delete="${i}">Delete</button></div>`).join(""):`<p>No measurements yet.</p>`}</div></section>`,"");
}
function bindHealth(){
 ...
 saveData();render()});
 document.querySelector("#addMeasure")?.addEventListener("click",()=>{data.measurements.push({date:document.querySelector("#measureDate").value||today(),waist:document.querySelector("#measureWaist").value,tummy:document.querySelector("#measureTummy").value});
 data.healthPromptLog=data.healthPromptLog||{};data.healthPromptLog[today()]=data.healthPromptLog[today()]||{};
 data.healthPromptLog[today()][new Date().getHours()<14?"morning":"evening"]=true;
 saveData();render()});
