...
function ensureHealthView(){
  ...
  data.healthView.measuresSection=["today","history","trends"].includes(data.healthView.measuresSection)?data.healthView.measuresSection:"today";
}
...
function HealthPage(){
 ...
 const measuresTabs=`<div class="module-section-tabs measures-section-tabs"><button type="button" data-measures-section="today" class="${measuresSection==="today"?"active":""}">Today</button><button type="button" data-measures-section="history" class="${measuresSection==="history"?"active":""}">History</button><button type="button" data-measures-section="trends" class="${measuresSection==="trends"?"active":""}">Trends</button></div>`;
 ...
 const measuresHistory=`<section class="card measures-history-card"><div class="section-title"><div><span class="section-kicker">Saved entries</span><h2>Weight history</h2></div><strong>${weights.length}</strong></div><div class="measure-history-list">${weightHistory||`<div class="health-graph-empty">No weight entries yet.</div>`}</div></section><section class="card measures-history-card"><div class="section-title"><div><span class="section-kicker">Saved entries</span><h2>Measurement history</h2></div><strong>${measures.length}</strong></div><div class="measure-history-list">${measurementHistory||`<div class="health-graph-empty">No measurement entries yet.</div>`}</div></section>`;
 const measuresTrends=`<section class="card measures-history-card"><div class="section-title"><div><span class="section-kicker">Trends</span><h2>Weight</h2></div><strong>${weights.length} entr${weights.length===1?"y":"ies"}</strong></div>${healthRangeButtons("weight",data.healthView.weightRange)}<div data-health-chart-panel="weight">${healthGraph(weights,[{label:"Weight",unit:"kg",get:healthWeightValue}],data.healthView.weightRange)}</div></section><section class="card measures-history-card"><div class="section-title"><div><span class="section-kicker">Trends</span><h2>Measurements</h2></div><strong>${measures.length} entr${measures.length===1?"y":"ies"}</strong></div>${healthRangeButtons("measurements",data.healthView.measureRange)}<div data-health-chart-panel="measurements">${healthGraph(measures,[{label:"Waist",unit:"cm",get:e=>e.waist},{label:"Tummy",unit:"cm",get:e=>e.tummy}],data.healthView.measureRange)}</div></section>`;
 const logTab=`${measuresTabs}${measuresSection==="history"?measuresHistory:measuresSection==="trends"?measuresTrends:measuresToday}`;
