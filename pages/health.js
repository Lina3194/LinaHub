...
function HealthPage(){
 ...
 const healthMetricIcon=kind=>kind==="weight"?`<svg class="health-metric-icon" viewBox="0 0 48 48" aria-hidden="true"><path d="M11 15.5h26l4 24H7l4-24Z"></path><path d="M16 15.5a8 8 0 0 1 16 0"></path><path d="M24 9.5v6"></path><path d="m24 15.5 4-3"></path></svg>`:`<svg class="health-metric-icon" viewBox="0 0 48 48" aria-hidden="true"><path d="M8 31c0-9.5 7.7-17.2 17.2-17.2S42 21.5 42 31v7H8v-7Z"></path><path d="M14 31v7M20 29v9M26 31v7M32 29v9M38 31v7"></path><path d="M12 17.5 8.5 14 12 10.5"></path><path d="M8.5 14h16"></path></svg>`;
 const measuresTabs=`<div class="module-section-tabs measures-section-tabs"><button type="button" data-measures-section="today" class="${measuresSection==="today"?"active":""}">Today</button><button type="button" data-measures-section="weight" class="${measuresSection==="weight"?"active":""}">${healthMetricIcon("weight")}<span>Trends (W)</span></button><button type="button" data-measures-section="measurements" class="${measuresSection==="measurements"?"active":""}">${healthMetricIcon("measurements")}<span>Trends (M)</span></button></div>`;
 const measuresToday=`<div class="health-form-grid"><section class="card compact-health-card"><h2 class="health-metric-heading">${healthMetricIcon("weight")}<span>Add weight</span></h2><div class="compact-health-fields"><input class="field" id="weightDate" type="date" value="${today()}"><input class="field" id="weightValue" type="number" step="0.1" placeholder="Weight kg"></div><button class="primary" id="addWeight">Save weight</button></section><section class="card compact-health-card"><h2 class="health-metric-heading">${healthMetricIcon("measurements")}<span>Add measurements</span></h2><input class="field" id="measureDate" type="date" value="${today()}"><div class="compact-health-fields"><input class="field" id="measureWaist" type="number" step="0.1" placeholder="Waist cm"><input class="field" id="measureTummy" type="number" step="0.1" placeholder="Tummy cm"></div><button class="primary" id="addMeasure">Save measurements</button></section></div>`;
