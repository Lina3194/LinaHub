...
function HealthPage(){
 ...
 <section class="card"><h2>Add measurements</h2><div class="two-col"><input class="field" id="measureDate" type="date" value="${today()}"><input class="field" id="measureWaist" type="number" step="0.1" placeholder="Waist cm"></div><div class="two-col" style="margin-top:10px"><input class="field" id="measureHips" type="number" step="0.1" placeholder="Hips cm"><input class="field" id="measureChest" type="number" step="0.1" placeholder="Chest cm"></div><div class="two-col" style="margin-top:10px"><input class="field" id="measureArm" type="number" step="0.1" placeholder="Arm cm"><input class="field" id="measureThigh" type="number" step="0.1" placeholder="Thigh cm"></div><button class="primary" id="addMeasure" style="margin-top:10px">Save measurements</button></section>
 ...
 <section class="card"><h2>Measurement history</h2><div class="list-card">${measures.length?measures.map((m,i)=>`<div class="item-row"><div><h3>${esc(formatDate(m.date))}</h3><p>Waist ${m.waist||"—"} · Hips ${m.hips||"—"} · Chest ${m.chest||"—"} · Arm ${m.arm||"—"} · Thigh ${m.thigh||"—"} cm</p></div><button class="mini danger" data-measure-delete="${i}">Delete</button></div>`).join(""):`<p>No measurements yet.</p>`}</div></section>`,"");
}
function bindHealth(){
 ...
 saveData();render()});
 document.querySelector("#addMeasure")?.addEventListener("click",()=>{data.measurements.push({date:document.querySelector("#measureDate").value||today(),waist:document.querySelector("#measureWaist").value,hips:document.querySelector("#measureHips").value,chest:document.querySelector("#measureChest").value,arm:document.querySelector("#measureArm").value,thigh:document.querySelector("#measureThigh").value});
 data.healthPromptLog=data.healthPromptLog||{};data.healthPromptLog[today()]=data.healthPromptLog[today()]||{};
 data.healthPromptLog[today()][new Date().getHours()<14?"morning":"evening"]=true;
 saveData();render()});
