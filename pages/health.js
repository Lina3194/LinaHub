...
function HealthPage(){
 ...
 const dashboard=`<section class="card health-dashboard-hero"><span class="section-kicker">❤️ Your health today</span><h2>${new Date().toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long"})}</h2><div class="health-dashboard-stats"><div><small>Sleep</small><strong>${latestSleep?minutesLabel(latestSleep.totalMinutes):"—"}</strong></div><div><small>Energy</small><strong>${morning.energy!=null?HEALTH_FEELINGS.energy[morning.energy][0]:"—"}</strong></div><div><small>Mood</small><strong>${morning.mood!=null?HEALTH_FEELINGS.mood[morning.mood][0]:"—"}</strong></div><div><small>Pain</small><strong>${morning.pain!=null?HEALTH_FEELINGS.pain[morning.pain][0]:"—"}</strong></div></div></section><section class="health-module-grid"><button data-health-open="sleep"><span>😴</span><strong>Sleep</strong><small>${latestSleep?minutesLabel(latestSleep.totalMinutes):"Add sleep"}</small></button><button data-route="medication"><span>💊</span><strong>Medication</strong><small>Today, history & stock</small></button><button data-route="period"><span>🌸</span><strong>Period</strong><small>Cycle & predictions</small></button><button data-health-open="weight"><span>⚖️</span><strong>Weight</strong><small>${latest?`${esc(healthWeightValue(latest))} kg`:"Add weight"}</small></button><button data-health-open="measurements"><span>📏</span><strong>Measurements</strong><small>${measures.length} entries</small></button><button data-health-open="garden"><span>🌷</span><strong>Flower Garden</strong><small>${todayFlowers.length} today</small></button></section>`;
 ...
 const gardenTab=`<section class="card flower-checkin-card"><div class="section-title"><div><span class="section-kicker">🌸 Through the day</span><h2>Energy, mood & pain</h2></div><strong>${new Date().toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}</strong></div><h3>Energy</h3>${circleScale("dayEnergy",HEALTH_FEELINGS.energy)}<h3>Mood</h3>${circleScale("dayMood",HEALTH_FEELINGS.mood)}<h3>Pain</h3>${circleScale("dayPain",HEALTH_FEELINGS.pain)}<button class="primary" id="saveDayFlower">Add flower</button></section><section class="card bouquet-card"><div class="section-title"><div><span class="section-kicker">Today’s colours</span><h2>Your energy bouquet</h2></div><strong>${todayFlowers.length} flowers</strong></div><div class="bouquet ${todayFlowers.length?"":"empty-bouquet"}">${todayFlowers.length?todayFlowers.map(flowerSvg).join(""):`<p>Your first check-in will grow the first flower.</p>`}<div class="bouquet-wrap"></div></div><div class="flower-key">${ENERGY_FLOWERS.map((c,i)=>`<span><i style="background:${c}"></i>${HEALTH_FEELINGS.energy[i][1]}</span>`).join("")}</div></section>`;
 ...
 return shell(`${head("Health","Your wellbeing in one gentle dashboard")}<nav class="health-tabs"><button class="${tab==='dashboard'?'active':''}" data-health-tab="dashboard">Overview</button>${tab!=='dashboard'?`<button class="active">${({sleep:'Sleep',garden:'Flowers',weight:'Weight',measurements:'Measurements',log:'Add'})[tab]||'Health'}</button>`:''}</nav>${content}`,"health");
}
...
function bindHealth(){
 ...
 document.querySelector('#saveDayFlower')?.addEventListener('click',()=>{const energy=selectedHealthFeeling('dayEnergy'),mood=selectedHealthFeeling('dayMood'),pain=selectedHealthFeeling('dayPain');if(energy===null||mood===null||pain===null){toast('Choose your energy, mood and pain');return}const stamp=entryTimestamp(today());data.dayCheckins=data.dayCheckins||[];data.dayCheckins.push({id:`flower-${Date.now()}`,...stamp,energy,mood,pain});saveData();toast('A new flower grew 🌸');render()});
