...
function bindHealth(){
 ...
 document.querySelectorAll("[data-health-range]").forEach(button=>button.onclick=()=>{
   const kind=button.dataset.healthRange,range=button.dataset.rangeValue;
   if(kind==="weight")data.healthView.weightRange=range;else data.healthView.measureRange=range;
   saveData();
   const section=document.querySelector(`[data-health-chart-section="${kind}"]`);
   if(!section)return;
   section.querySelectorAll(`[data-health-range="${kind}"]`).forEach(item=>item.classList.toggle("active",item.dataset.rangeValue===range));
   const panel=section.querySelector(`[data-health-chart-panel="${kind}"]`);
   if(!panel)return;
   const entries=kind==="weight"?[...(data.weightEntries||[])].sort((a,b)=>healthSortValue(b).localeCompare(healthSortValue(a))):[...(data.measurements||[])].sort((a,b)=>healthSortValue(b).localeCompare(healthSortValue(a)));
   const series=kind==="weight"?[{label:"Weight",unit:"kg",get:healthWeightValue}]:[{label:"Waist",unit:"cm",get:e=>e.waist},{label:"Tummy",unit:"cm",get:e=>e.tummy}];
   panel.classList.add("is-updating");
   requestAnimationFrame(()=>{
     panel.innerHTML=healthGraph(entries,series,range);
